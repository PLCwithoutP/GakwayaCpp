...
// This causes no problems because we are working with
...
{
    // Compiles but won't allow to access data
...
int main()
{
    const Dog dog1("mcFluffy", "mcShepherd", 1);
    ...
    //function_taking_const_dog_ref(dog1); // Compiles but won't allow to access data

    //function_taking_dog_p(&dog1); // Compiler error

    //function_taking_pointer_to_const_dog(&dog1); // Compiles but won't allow to access data
    return 0;
}