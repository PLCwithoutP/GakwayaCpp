...
/*     Dog* p_dog2 = new Dog("Dog2");
    ...
    // If you go out of scope without releasing p_dog2 and p_int1
    // you will have leaked memory
    delete p_dog2; // Calls the destructor
    delete p_int1; */

    // ------------------------------------------------------------- //
    // Using unique pointer: managed memory is owned by a single pointer at any moment
/*     {
            Dog* p_dog_3 = new Dog("Dog3");
        std::unique_ptr<Dog> up_dog_4 {p_dog_3}; // Can also manage a previously allocated
                                                // space managed by a raw pointer. You 
                                                // should not try to use the raw pointer
                                                // from this point on
        std::unique_ptr<Dog> up_dog_5 {new Dog("Dog5")};
        std::unique_ptr<int> up_int_2 {new int(200)};
        std::unique_ptr<Dog> up_dog_6 {nullptr}; // Can also initialize with nullptr
                                                // and give it memory to manage later, we will see
                                                // how to do that with std::move later in the lecture.
                                                // Just know that you can initialize a unique ptr with
                                                // nullptr for now.
        ...
        *up_int_2 = 500;
        std::cout << "Integer is : " << *up_int_2 << std::endl; // dereferencing
        std::cout << "Integer lives at address : " << up_int_2.get() << std::endl;
    } */

    // ------------------------------------------------------------- //
    // Using make_unique syntax, much cleaner (C++-14)
    // Calls new internally for us, we do not have to do it ourselves
/*     {
        std::unique_ptr<Dog> up_dog_7 = std::make_unique<Dog>("Dog7");
        up_dog_7 -> print_dog();

        std::unique_ptr<int> p_int_3 = std::make_unique<int>(30);
        *p_int_3 = 78;
        std::cout << "Value pointed to by p_int_3 is : " << *p_int_3 << std::endl;
        std::cout << "p_int pointing at address : " << p_int_3.get() << std::endl;
    }
 */
    // ------------------------------------------------------------- //
/*     // Copies are not allowed
    ...
    //std::unique_ptr<Dog> up_dog_9 = up_dog_8; // Error. This also does some kind of copy
                                              // More on this when we've learnt about operator
                                              // overloading
    //std::unique_ptr<Dog> up_dog_10 {up_dog_9}; // Error : Copy constructor deleted */
...
/*     // Can however move the pointer
    ...
    // destructor is called before the line :
    // std::cout << "Doing something" << std::endl;
    {
        std::unique_ptr<Dog> up_dog_13 = std::make_unique<Dog>("Dog13");
        up_dog_13.reset(); // releases the memory and sets pointer to nullptr
